...
checkconfigs = {
    "build", "test/config-crossref", "test/config-nomencl",
    "test/config-bibtex", "test/config-biblatex"
}
...
function update_tag(file, content, tagname, tagdate)
    ...
    if string.match(file, "%.cls$") then
        ...
        if string.match(content,
                        "\\ProvidesClass{ustcthesis}%[%d%d%d%d/%d%d/%d%d") then
            content = string.gsub(content,
                                  "\\ProvidesClass{ustcthesis}%[%d%d%d%d/%d%d/%d%d",
    ...
    elseif string.match(file, "CHANGELOG.md") then
        ...
        content = string.gsub(content, "## %[Unreleased%]",
                              "## [Unreleased]\n\n## [" .. tagname .. "] - " ..
                                  tagdate)
        content = string.gsub(content, previous .. "%.%.%.HEAD",
                              tagname .. "...HEAD\n[" .. tagname .. "]: " .. url ..
                                  "/compare/" .. previous .. "..." .. tagname)
    end
    return content
end
