...
\ustcsetup{
  keywords = {
      时间信息抽取, 中文时间表达式, 统计句法分析
    },
  keywords* = {
      Temporal Information Extraction, Chinese Temporal Expression, Statistical parsing
    },
}

\begin{abstract}

  自然语言处理领域中，时间和日期作为一类重要的命名实体和语义载体而存在，在问答系统、多轮对话、信息检索等任务中有着广泛的应用。
  因为中文本身的复杂性和采用标注格式的局限性，现有的时间信息抽取解决方案产生的结构化时间数据都不尽如人意，不能较好的表现时间实体的内在语义。
  本文主要基于概率上下文无关语法解决中文时间信息抽取中的这一系列问题。
  在此基础上，论文旨在设计并实现一个完整的中文时间信息抽取系统，以提升时间处理模块作为其他自然语言处理任务组件的使用体验。

  本文首先对中文时间信息抽取的一系列痛点问题做出分析, 将中文时间信息抽取分解为识别和解析两个具体需求, 并基于文档语料的特点的采用MongoDB存储数据。
  其次, 采用分层架构的思想将系统分为四层架构, 并依据四层架构的特点设计出功能明晰的不同模块。
  然后, 通过上下文无关语法的形式简化传统方法中手工驱动编写时间表达式规则的繁琐过程，并采用统计学习方法解决识别出的时间信息可能存在的歧义性冲突。
  对于识别出的时间信息，再利用时间粒度天然具有层次性和顺序性的语义特征，构建时间信息的语义表示，以改善主流方法中归一化后的时间信息抽取格式的局限性。
